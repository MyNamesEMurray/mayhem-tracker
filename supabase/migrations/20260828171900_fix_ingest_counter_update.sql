-- The counter update in the previous migration was two statements with a
-- case expression comparing a variable to the column it was setting. It
-- happened to work, but it was unreadable and one edit away from wrong. The
-- row is held with FOR UPDATE, so v_stats already holds the rolled window and
-- the values can just be written.
...
begin
  ...
  -- Quarantine, honouring the flood cap. Anything past the cap is reported
  ...
  -- Counted on what was actually written, so re-uploading a batch does not
  -- spend an allowance on games already contributed. v_stats holds the rolled
  -- window and the row is locked, so these are the values to write.
  update public.contributor_stats
     set lifetime_games     = v_stats.lifetime_games + v_inserted + v_quarantined_now,
         window_started_at  = v_stats.window_started_at,
         window_games       = v_stats.window_games + v_inserted + v_quarantined_now,
         pending_quarantine = v_stats.pending_quarantine + v_quarantined_now,
         updated_at         = now()
   where contributor_token = p_token;
