...
- **Augments that work together.** Expand an augment on the Augments tab and, next to the champions it's best on, you'll see what it pairs well with - and how far the pair beats the better of the two on its own, which is the part that isn't just "this augment is good on its own". Nobody else records Mayhem augments at the grain this needs, so it isn't a number you can go and check somewhere else. [MayhemStats.com](https://mayhemstats.com/) shows it too.
